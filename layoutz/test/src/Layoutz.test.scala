...
Third line"""
  ...
  ◦ Frontend
    ...
    test("Key ADT construction and pattern matching") {
      ...
      charKey match {
        case CharKey(c) => assert(c == 'a')
      }

      specialKey match {
        case SpecialKey(name) => assert(name == "F1")
      }

      assert(enterKey == EnterKey)
      assert(escapeKey == EscapeKey)
    }
...
[LAYOUT] Second line"""
    ...
    test("key parsing") {
      ...
      assert(KeyParser.parse(65, mockTerminal) == CharKey('A'))
      assert(KeyParser.parse(97, mockTerminal) == CharKey('a'))
      assert(KeyParser.parse(43, mockTerminal) == CharKey('+')) // Plus sign - ASCII 43
      assert(KeyParser.parse(45, mockTerminal) == CharKey('-')) // Minus sign - ASCII 45

      // Test special keys
      assert(KeyParser.parse(10, mockTerminal) == EnterKey)
      assert(KeyParser.parse(13, mockTerminal) == EnterKey)
      assert(KeyParser.parse(9, mockTerminal) == TabKey)
      assert(KeyParser.parse(127, mockTerminal) == BackspaceKey)

      // Test control characters
      assert(KeyParser.parse(1, mockTerminal) == SpecialKey("Ctrl+A"))

      // Test arrow keys with mock terminal (fallback path)
      mockTerminal.queueInput("[A") // ESC sequence for up arrow
      assert(KeyParser.parse(27, mockTerminal) == ArrowUpKey)

      mockTerminal.queueInput("[B") // ESC sequence for down arrow
      assert(KeyParser.parse(27, mockTerminal) == ArrowDownKey)

      mockTerminal.queueInput("[C") // ESC sequence for right arrow
      assert(KeyParser.parse(27, mockTerminal) == ArrowRightKey)

      mockTerminal.queueInput("[D") // ESC sequence for left arrow
      assert(KeyParser.parse(27, mockTerminal) == ArrowLeftKey)
    }
    ...
    test("counter app keys work") {
      ...
      assert(KeyParser.parse(43, mockTerminal) == CharKey('+'))
      assert(KeyParser.parse(45, mockTerminal) == CharKey('-'))
      ...
      assert(plusResult == CharKey('+'))
      assert(minusResult == CharKey('-'))
    }
