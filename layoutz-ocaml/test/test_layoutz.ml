...
let test_line_break () = check string "line break renders empty" "" (render br)
let test_text_width () = check int "text width" 5 (width (text "Hello"))
let test_text_height () = check int "text height" 1 (height (text "Hello"))

let test_multiline_height () =
  check int "multiline height" 2 (height (text "Line 1\nLine 2"))

let test_hr_default_width () = check int "hr default width" 50 (width hr)

let test_hr_custom_width () =
  check int "hr custom width" 10 (width (hr' ~width:10 ()))

let test_hr_custom_char () =
  check string "hr custom char" "=========="
    (render (hr' ~char:"=" ~width:10 ()))
...
let test_tight_row () =
  check string "tight row" "AB"
    (render (row ~tight:true [ text "A"; text "B" ]))
...
let test_nested_layout () =
  let nested =
    layout
      [
        text "Header";
        box ~title:"Box" [ text "Inside"; row [ text "A"; text "B" ] ];
        text "Footer";
      ]
  in
...
let test_box_utf8_border () =
  ...
  check bool "box top-left corner is ┌" true (String.sub output 0 3 = "┌");
...
let test_box_with_title_utf8 () =
  ...
  (* ┌─Hi─┐ or similar *)
  check bool "titled box starts with ┌" true (String.sub first_line 0 3 = "┌");
  check bool "titled box contains title" true (String.length first_line > 0)
...
let test_hr_utf8 () =
  ...
  check bool "hr uses ─ character" true (String.sub output 0 3 = "─")
...
let test_table_content () =
  ...
  let contains_alice =
    let rec check i =
      if i > String.length output - 5
      then false
      else if String.sub output i 5 = "Alice"
      then true
      else check (i + 1)
    in
    check 0
...
let test_row_with_boxes () =
  let output =
    render (row [ box ~title:"A" [ text "L" ]; box ~title:"B" [ text "R" ] ])
  in
  let output_lines = lines output in
  (* Row should have boxes side by side, both should have ┌ *)
  let first_line = List.hd output_lines in
  check bool "row has two box corners" true (String.length first_line > 10)
...
let nested_tests = [ ("nested layout", `Quick, test_nested_layout) ]
...
let test_unordered_list () =
  let output =
    render (ul [ li (s "First"); li (s "Second"); li (s "Third") ])
  in
  let output_lines = lines output in
  check int "ul has 3 lines" 3 (List.length output_lines);
  check bool "ul starts with bullet" true
    (String.sub (List.hd output_lines) 0 3 = "•")
...
let test_ordered_list () =
  let output =
    render (ol [ li (s "First"); li (s "Second"); li (s "Third") ])
  in
  let output_lines = lines output in
  check int "ol has 3 lines" 3 (List.length output_lines);
  check bool "ol first line starts with 1" true
    (String.sub (List.hd output_lines) 0 2 = "1.")
...
let test_nested_list () =
  ...
  in
  let output_lines = lines output in
  check bool "nested list has multiple lines" true
    (List.length output_lines >= 3)
...
let operator_tests = [ ("style compose", `Quick, test_style_compose) ]
...
let test_tree_basic () =
  ...
  check bool "tree root is first" true
    (String.sub (List.hd output_lines) 0 4 = "root")
